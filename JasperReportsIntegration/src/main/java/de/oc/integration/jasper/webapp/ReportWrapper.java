...
public class ReportWrapper extends HttpServlet {
	...
	Map<String, Object> filterInternalParameters(Map requestParams) {
		...
		while (iter.hasNext()) {
			...
			String mapKey = n.getKey().toString();
			...
			if (!mapKey.startsWith("_")) {
				...
				if (mapKey.startsWith("i")) {
					reportParams.put(key, Integer.valueOf(values[0].toString()));
				} else if (mapKey.startsWith("b")) {
					reportParams.put(key, new BigDecimal(values[0].toString()));
				} else {
	...
	public void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		logger.info("real path: " + getServletContext().getRealPath(""));
		...
		try {
			...
			isValidHash = Utils.validateHash(urlCallInterface.signStr, appConfig.signHash, request.getHeader("Gx0s1"));
			if (!isValidHash) {
				Utils.throwRuntimeException("Invalid hash");
			}
			...
			if (urlCallInterface.outFilename != null && !"".equals(urlCallInterface.outFilename)) {
				response.setHeader("Content-Disposition", "inline; filename=" + urlCallInterface.outFilename);
			}
			...
			if (urlCallInterface.saveIsEnabled.booleanValue()) {
				...
				if (appConfig.saveFileIsEnabled) { 
					...
					logger.info("Dirname null? " + (dirName == null ? "Yes" : "No (" + dirName + ")"));
					
					if (dirName == null) {
						urlCallInterface.saveFileName = getServletContext().getRealPath("") + "r\\" + urlCallInterface.saveFileName;
						file = new File(urlCallInterface.saveFileName);
			...
			if (out != null) {
				if (isValidHash) {
					...
					response.setLocale(new Locale(urlCallInterface.repLocale));
					// found this to be mandatory with IE 6.0
					response.setHeader("content-type", "text/plain" + "; charset=" + urlCallInterface.repEncoding);
					response.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
					
					String message = "Invalid hash";
